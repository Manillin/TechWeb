...
class SearchForm(forms.Form):
    ...
    CHOICE_LIST = [("Questions", "Search in Questions"),  # valore inviato al server - etichetta visualizzata sul form
                   ("Choices", "Search in Choices")]

    search_string = forms.CharField(
        label="Search String", max_length=100, min_length=3, required=True)
    search_where = forms.ChoiceField(
        label="Search Where?", required=True, choices=CHOICE_LIST)


class VoteForm(forms.Form):

    answer = forms.ModelChoiceField(
        queryset=None, required=True, label="Select your answer!")
...
class CreateQuestionForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (len(self.cleaned_data["question_text"]) < 5):
            self.add_error(
                "question_text", "Error: question text must be at least 5 characters long")
